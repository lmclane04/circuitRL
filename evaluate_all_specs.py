...
def load_agent(agent: str, run_dir: str, spec_pool_test: str, config_override: str | None = None):
    ...
    if chosen_agent != "bo" and checkpoint is None:
        raise FileNotFoundError(
            f"No model.pt found in {run_dir}. "
    ...
    # BO agent: no neural network, load from pickle
    if chosen_agent == "bo":
        from circuitrl.agents.bo_agent import BOAgent
        bo_agent = BOAgent(env, config)
        bo_agent.load(checkpoint_path)
...
def main():
    ...
    # BO agent: deterministic best point, evaluate once per spec in pool
    if chosen_agent == "bo":
        bo_results = network_or_agent.evaluate(n_episodes=len(env._spec_pool))
