...
# ---------------------------------------------------------------------------
# Sequential PPO — one parameter moved per step.
# ---------------------------------------------------------------------------

class SeqActorCritic(nn.Module):
    """Shared-trunk actor-critic with a single policy head.

    A one-hot encoding of the current parameter is part of the observation, 
    so the single head conditions on which parameter it is deciding this step.
    """
    ...
    def __init__(self, obs_dim: int, hidden: int = 64):
        ...
        self.policy_head = nn.Linear(hidden, 3)  # {decrease, no-op, increase}
        self.value_head = nn.Linear(hidden, 1)

    def get_action(self, obs: torch.Tensor):
        """Sample one action. Returns (action, log_prob, value)."""
        h = self.trunk(obs)
        dist = Categorical(logits=self.policy_head(h))
        action = dist.sample()
        return action, dist.log_prob(action), self.value_head(h).squeeze(-1)

    def evaluate(self, obs: torch.Tensor, actions: torch.Tensor):
        """Evaluate actions. Returns (log_prob, entropy, value)."""
        h = self.trunk(obs)
        dist = Categorical(logits=self.policy_head(h))
        return dist.log_prob(actions), dist.entropy(), self.value_head(h).squeeze(-1)


class SeqRolloutBuffer:
    """Rollout buffer for scalar actions."""
...
class PPOSeqAgent:
    def __init__(self, env, config: dict):
        ...
        # Same as before (pretty standard hyperparameter choice for PPO)
        self.clip_eps = 0.2
        self.vf_coef = 0.5
        self.ent_coef = 0.01
        self.gae_lambda = 0.95
        ...
        self.network = SeqActorCritic(obs_dim)
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.lr)
        self.buffer = SeqRolloutBuffer(self.n_steps, obs_dim)
    ...
    def collect_rollouts(self) -> list[dict]:
        ...
        for _ in range(self.n_steps):
            ...
            self.buffer.store(obs, action_int, log_prob.item(), reward, float(done), value.item())
            ep_reward += reward
            ep_len += 1
    ...
    def save(self, path: str):
        torch.save({
            "seq_network": self.network.state_dict(),
            "optimizer": self.optimizer.state_dict(),
        }, path)

    def load(self, path: str):
        checkpoint = torch.load(path, weights_only=True)
        self.network.load_state_dict(checkpoint["seq_network"])
        self.optimizer.load_state_dict(checkpoint["optimizer"])
