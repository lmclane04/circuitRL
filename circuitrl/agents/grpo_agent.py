...
class GRPOAgent:
    ...
    def _rollout(self, start_indices: np.ndarray,
                 start_targets: np.ndarray | None = None) -> tuple[list, float, dict]:
        """Run one episode from start_indices (and optionally fixed start_targets).

        When specs are randomised, all rollouts in a group must share the same
        target so that their returns are on the same scale and group-relative
        ...
        """
        # Reset picks random params + random target if randomise_specs is on
        self.env.reset()
        # Override with the shared group start state
    ...
    def train(self, total_timesteps: int | None = None, callback=None):
        ...
        while timesteps_done < total:
            ...
            self.env.reset()
            start_indices = self.env._param_indices.copy()
